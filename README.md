...
### 过程合理的重要性
只在意结果,不在意过程,并认为过度描述过程是在推卸责任.那么问题来了

- 场景一
  
  由于用户操作,导致数据错误.用户投诉电话打过来,想让运营改一下数据.这两个人想的肯定是让数据变得正确.那么问题就是,**过程都是不合理的**(人工手动组篡改数据),那么结果又怎么可能是合理的呢?

- 场景二: 体制的错误

  由于贪官,导致衙门判决结果的错误,给无罪人员判了死刑,乾隆皇帝微服私访,看到了,就下了一道圣旨,特赦了无辜百姓,严惩了贪官.到此为止,一段佳话流传出来....但是其实体制是否形同虚设,到底是法制还是人治,结果对了,就正确了吗?人治干预法制,连**过程**都是错的,那**结果**又怎么可能会是对的呢??我们只看重结果,那么结果又是什么?应该要完善体制的完备性,而不应该过多的干预市场.这就是(体制化/工程化)的重要性.

- 结论
  看到数据不对,不应该手动去改数据,而应该完善整个工程化.